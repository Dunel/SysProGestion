...
export async function ProfileEstudentGet(req: NextRequest) {
  try {
    ...
    const object = {
      ...
      cneRegister: profile?.cneRegister,
      cneCentroName: profile?.cneCentroName,
      cneParroquia: profile?.cneParroquia,
    };
...
export async function ProfileAlcaldiaUpdate(req: NextRequest) {
  try {
    const token = await getToken({ req, secret: process.env.NEXTAUTH_SECRET });
    if (!token) {
      return NextResponse.json({ error: "No autorizado" }, { status: 401 });
    ...
    await prisma.alcaldiaInfo.upsert({
      where: { userCedula: cedula },
      update: {
        name: result.name,
        description: result.description,
...
export async function ProfileAlcaldiaGet(req: NextRequest) {
  try {
    const token = await getToken({ req, secret: process.env.NEXTAUTH_SECRET });
    if (!token) {
      return NextResponse.json({ error: "No autorizado" }, { status: 401 });
    ...
    const cedula = token.cedula;
    const profile = await prisma.alcaldiaInfo.findFirst({
      where: { userCedula: cedula },
      select: {
        name: true,
        description: true,
