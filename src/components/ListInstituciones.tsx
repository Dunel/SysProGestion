...
export default function ListInstituciones() {
  ...
  const typeInstitu = [
    { name: "universitaria", text: "Universitaria" },
    { name: "tecnica", text: "Técnica" },
  ];
  const [typeOpen, setTypeOpen] = useState(false);
  const [institutionToDelete, setInstitutionToDelete] = useState<number | null>(
    null
  );
  const {
    register,
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      if (editingId) {
        ...
        setInstitution(
          institution.map((reg) => (reg.id === res.data.id ? res.data : reg))
        );
        handleCancelEdit();
      }
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      setLoading(true);
      await axios.delete(`/api/alcaldia/institutions`, {
        data: { id: institutionToDelete },
      });
      await getInstitu();
    } catch (error) {
      if (axios.isAxiosError(error)) {
              ...
              <TableCell>
                <>
                  <Input
                    ...
                    type="text"
                    value={
                      typeInstitu.find((e) => e.name === watch("type"))?.text ||
                      ""
                    }
                      ...
                      {typeInstitu.map((e, index) => (
                        <div
                          ...
                          onClick={() => {
                            setValue(
                              "type",
                              e.name as "universitaria" | "tecnica"
                            );
                            setTypeOpen(false);
                          }}
                ...
                {editingId === insttt.id ? (
                  <>
                    <Input
                      ...
                      type="text"
                      value={
                        typeInstitu.find((e) => e.name === watch("type"))
                          ?.text || ""
                      }
                      onClick={() => setTypeOpen(!typeOpen)}
                      readOnly
